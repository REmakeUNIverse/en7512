...
static struct mtd_partition tc3162_parts[] = {
	{									 	/* 1th partition */
		  name 	     : BOOT_PART,	 	        /* Bootloader section */ 
		  size	  	 : KERNEL_OFFSET, 		 	/* Size  */
		  offset     : 0				 	/* Offset from start of flash- location 0x0*/
//		  mask_flags : MTD_WRITEABLE	 	/* This partition is not writable */
	},
	{									 	/* 2th partition */
		  name       : KERNEL_PART,		 	/* Kernel section */
#ifdef CONFIG_MTD_PURE_BRIDGE
		  size	     :  0x000a0000,		 	/* Size */
#else
		  size		 :	0x00150000,
	...
	{									 	/* 3th partition */
		  name       : ROOTFS_PART, 		 	/* Root filesystem section */
		  size	     : 0x00680000,	        /* Size */ 
		  offset     : MTDPART_OFS_APPEND 	/* Append after kernel section */
	},
	{ 									    /* 5th partition */ 
		  name	     : CONFIG_PART,			/* romfile filesystem section */ 
		  size	     : 0x00010000,	        /* Size */ 
		  offset	 : MTDPART_OFS_APPEND   /* Append after kernel section */ 
	},
	{                                       /* 4th partition */  
		  name       : ROMFILE_PART,           /* config filesystem section */   
		  size       :  0x00010000,         /* Size */  
		  offset     : MTDPART_OFS_APPEND   /* Append after bootloader section */  
	}
};
...
static int tc3162_mtd_part_init()
{
	unsigned long offset = 0;
	unsigned char fileTagBuff[1496];
	unsigned long *fileTag = (unsigned long *)fileTagBuff;
	int ret = 0, retlen = 0;
	struct mtd_partition *pmtdPart = NULL;
	unsigned long maxrootfssize = 0;
	int i;	
	
	if (!tc3162_mtd_info)
	{
		printk("Invalid tc3162 mtd info!\n");
		return -1;
	}

	if (!tc3162_mtd_info->read)
	{
		printk("No read function in tc3162 mtd info!\n");
		return -1;
	}

	offset = KERNEL_OFFSET;
	memset(fileTag, 0, 1496);
	ret = tc3162_mtd_info->read(tc3162_mtd_info, offset, 1496, &retlen, fileTag);
	if (ret)
	{
		printk("mtd read error, ret = %d\n", ret);
		return ret;
	}	

	maxrootfssize = CONFIG_OFFSET - KERNEL_OFFSET - 1496 - fileTag[277];

	for (i = 0; i < sizeof(tc3162_parts) / sizeof(tc3162_parts[0]); i++)
	{
		if (!strcmp(tc3162_parts[i].name, KERNEL_PART))
		{
			tc3162_parts[i].size = fileTag[277] + 1496;
		}
		else if (!strcmp(tc3162_parts[i].name, ROOTFS_PART))
		{
			tc3162_parts[i].size = maxrootfssize;
			printk("rootfs length is %llx", tc3162_parts[i].size);
		}
	}
...
#ifdef CONFIG_TP_IMAGE
	if(ret = tc3162_mtd_part_init()){
		printk("tc3162_mtd_part_init() fail\n");
		return ret;
	}
	add_mtd_partitions(tc3162_mtd_info, tc3162_parts, tc3162_parts_size);
